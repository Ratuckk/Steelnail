...
[State -1, Thorns]
type = ChangeState
triggerall = roundstate = 2 && time > 10
triggerall = power > 1000; && numhelper(300) = 0
trigger1 = command = "b"  && !ailevel  && command != "recovery"
triggerall = statetype = A && stateno = [5000,5500] 
trigger2 = life > enemy,life  ;. || (stateno != 300 &&  (stateno = [ 200,210]) && pos Y  = 0 && ((enemy,movereversed || movereversed)) && time > 5)
trigger2 = ailevel && random = [0,20]
trigger3 = random < 200
trigger3 = ailevel && enemy,GetHitVar(hitcount) >= 11 
value = 229
...
triggerall = power > 1500 && numhelper(300) = 0
trigger1 = command = "c"  && !ailevel 
triggerall = ctrl || stateno = [5000,5500] 
trigger2 = life > enemy,life  ;. || (stateno != 300 &&  (stateno = [ 200,210]) && pos Y  = 0 && ((enemy,movereversed || movereversed)) && time > 5)
trigger2 = ailevel && random = [0,10]
value = 223


[State -1, AI Counter]
type = ChangeState
triggerall = var(35) = 0 && var(40) >= 10
value = cond(random < 500,215, 421)
;------------   
triggerall = ailevel
triggerall = RoundState = 2 && p2movetype = A && pos Y = 0 && stateno != 421; && enemy,fvar(39) > 0
triggerall = ctrl  || stateno = 422  && time > 5 ||  (stateno = 100) 
trigger1 = p2bodydist X = [-30,120+(enemy,fvar(39) + var(45))]
...
triggerall = alive && command = "a"  && command != "holddown"
trigger1 = ctrl  || stateno = [110,111]
value = 114
 

[State -1, Down dash]
type = selfState
triggerall = statetype = A || pos Y < -40 || stateno = [105,119]
triggerall = alive && !ailevel && stateno != 114 && command = "holddown" && command = "a" 
trigger1 = ctrl || stateno = [110,111]
value = 65
...
triggerall = var(35) = 0 
triggerall = ailevel
triggerall = statetype = A && ctrl || stateno = [105,119] 
triggerall = pos Y < -30
...
trigger1 = (p2movetype = A && enemy,pos Y < 0) || p2movetype = A && p2bodydist X < -20 
trigger2 = (enemy,vel X >= 7) && p2bodydist Y < 0 && p2movetype = A 
value = 65;cond(command = "holdback",114,115) 
...
type = ChangeState
value = 221 + (var(6) = 1)*1
triggerall = var(35) = 0 && var(40) >= 0 && power > 500
trigger1 = command = "z" && command = "holddown" && command = "holdback" && !ailevel
triggerall = ctrl || (stateno = [100,101]) || (stateno = [201,210]) && movecontact || stateno = [110,111]
trigger2 = (p2bodydist X = [-60,60]) && p2bodydist Y < -20
trigger2 = ailevel 
 
triggerall = stateno != [221,222]
...
value = 203 + (var(6) = 1)*3
triggerall = var(29) = 0
triggerall = var(35) = 0 && stateno != [203,206]
trigger1 = command = "y" && !ailevel
trigger1 = command = "holddown" && command = "holdfwd"
triggerall = ctrl || (stateno = [100,101]) || (stateno = [201,210]) && movecontact || stateno = [110,111]

trigger2 = ailevel && random = [420,580 + (p2statetype = L)*100] 
trigger2 = p2movetype != A && p2bodydist X = [100,180 + (Vel X * 3)]
trigger2 = p2bodydist Y = [-40,20]


[State -1, Descending STOMP]
type = ChangeState
value = 213 + (var(6) = 1)*2
triggerall = var(35) = 0 && var(40) = 0
trigger1 = command = "z" && command = "holddown"  && !ailevel
triggerall = (pos Y < -50 && ctrl) || stateno = 260 || movehit && stateno = 202 || movehit && (stateno = [201,205])  || stateno = [110,111]
trigger2 =  (p2bodydist X = [-35,35])   && p2bodydist Y >= 50
trigger2 = ailevel 
 
triggerall = stateno != [213,215]
...
triggerall = ctrl || stateno = 69 && time > 4|| stateno = 67 && time > 3 || stateno = [110,115]
trigger2 = ailevel
trigger2 =  (p2bodydist X = [-50,50])  && p2bodydist Y = [-10,50] 
 
[State -1, Up attack]
type = ChangeState
value = 207
triggerall = var(35) = 0 && stateno != 207
trigger1 = pos Y = 0 && (command = "x" && command = "holddown") || pos Y < 0 && (command = "x" && command = "holdup") 
...
trigger2 = ailevel
trigger2 = (p2bodydist X  = [-60, 60 + (Vel X * 5)]) ;|| vel X > 8 && time > 4 && stateno = [100,101] 
...
triggerall = ctrl || (stateno = [100,101]) || (stateno = [201,210]) && movehit  ||  var(19) = 1  ;|| ((enemy,var(18) = 1) && (stateno != [300,422]) &&  ((stateno = [ 200,201]) || (stateno = [204,210])) && pos Y  = 0 && ((enemy,movereversed || movereversed)) && time > 5)
trigger2 = ailevel && random = [100,140 + (p2statetype = L)*70 + vel X*3 + ((enemy,vel X < -3) && p2movetype != A)*35]
...
type = ChangeState
value = 217 
triggerall = var(29) = 0 && var(40) = 0 && var(35) = 0 && stateno != 215
...
trigger2 = p2bodydist X = [150-(p2movetype = H)*80,950] 
...
trigger2 =  p2bodydist X  = [0, 90 + (Vel X * 3)]
trigger2 = p2bodydist Y = [-20,20]
trigger2 = var(40) = 0
...
type = ChangeState
value = 210
triggerall = var(29) = 0 && var(35) = 0 && var(40) = 0 && pos Y = 0
trigger1 = command = "y" && command != "holddown" && !ailevel 
triggerall = ctrl || (stateno = [100,101])  || stateno = 202 && movehit && animelemno(0) > 5
trigger2 = ailevel && p2movetype != A && random = [760,888]
trigger2 =  p2bodydist X  = [0,210 + (Vel X * 3)]  
...
type = changestate
triggerall = var(35) = 0 
Value = cond((random < 500 && pos Y = 0),421,(105+(pos Y < 0)*6)) ;This value is based on default MUGEN common1.cns "Hop backwards" state.
triggerall = ailevel
triggerall = ctrl
...
triggerall = pos Y = 0 && statetype = S
triggerall = roundstate = 2
triggerall = ailevel && stateno != [100,101]
...
trigger2 = random < 240 + (p2movetype = A && enemy,vel X>=6)*50 
...
[State 0, Guard]
type = ChangeState
trigger2 =var(29) >=0 ||  var(40) > 0
triggerall = abs(enemynear,pos X - pos X) = [0,70]
triggerall = ailevel && roundstate = 2 && var(35) = 0 && pos Y = 0 &&(stateno != [120,150])
triggerall = ctrl && enemynear,movetype = A ;||  animtime >= -9 && (enemynear,movetype = A) && p2statetype != L && movetype = I || stateno = 5001 && time > 10
trigger1 = numhelper(33333333) = 1
trigger1 = PlayerIdExist(helper(33333333),var(3)) ;enemy projectile exists
trigger1 = PlayerID(helper(33333333),var(3)), pos y > -100 || PlayerID(helper(33333333),var(3)), vel y > 0 ; Avoid attempt to reflect projectiles that are to high. 100 = char height or reflector height, unless the enemy projectile is moving downwards.
trigger1 = PlayerID(helper(33333333),var(3)), vel x != 0 ; Avoid "Division by Zero" debug overflow
